...
- `cache.py` — 起動時に事前生成 → ffmpeg で **24kHz mono MP3、ID3/Xing なし**（デバイスは audioStreamingUrl を生の MP3 フレーム列として読むため、先頭にタグがあると同期できず無音になる）→ `data/songs/`。**リアルタイム合成はしない**。楽譜・style_id・ENGINE バージョン・出力フォーマットのハッシュがキャッシュキー
